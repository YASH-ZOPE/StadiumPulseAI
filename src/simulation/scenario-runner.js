...
export class ScenarioRunner {
  ...
  async runConnectedDemo() {
    ...
    try {
      ...
      log.push(
        await this._step(1, 'Gate B crowd surge', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
      ...
      log.push(
        await this._step(2, 'Gate B queue exceeds 24 min', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
      ...
      log.push(
        await this._step(3, 'Thunderstorm warning issued', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
      ...
      log.push(
        await this._step(4, 'East concourse accessible corridor closed', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
          ...
          this._bus.emit(
            ...
            createEvent({
              ...
              payload: {
                from: 'concourse-east',
                to: 'sensory-room',
                reason: 'Weather shelter setup blocking corridor',
              },
            }),
          );
        }),
      ...
      log.push(
        await this._step(5, 'Medical incident reported at Food Court NE', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
              ...
              payload: {
                type: 'medical',
                detail: 'Fan collapsed — suspected heat-related illness',
      ...
      log.push(
        await this._step(6, 'Gate A crowd rising (displaced from Gate B)', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
      ...
      log.push(
        await this._step(7, 'Transit hub pressure increasing', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
      ...
      log.push(
        await this._step(8, 'Fan sentiment trending negative', () => {
          this._bus.emit(
            'event:new',
            createEvent({
              source: 'simulation',
      ...
      /* Step 9 */
      log.push(
        await this._step(9, 'Risk analysis + cascade simulation triggered automatically', () => {}),
      );
      ...
      log.push(
        await this._step(
          10,
          'AI coordinated response generated — awaiting operator approval',
          () => {},
        ),
      );
      ...
      /* Step 11 */
      log.push(
        await this._step(11, 'Decision logged to audit timeline — scenario complete', () => {}),
      );
      ...
      return {
        ...
        log,
        message:
          'Connected scenario complete. Check the decision panel to approve AI recommendations.',
      };
    } finally {
      this._running = false;
    }
