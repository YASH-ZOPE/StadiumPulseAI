...
export function attachWsHandler(wss, ctx) {
  wss.on('connection', (ws) => {
    ...
    unsubs.push(
      ctx.bus.on('state:changed', (diff) => {
        send(ws, 'state:diff', { ...diff, snapshot: ctx.state.getSnapshot() });
      }),
    );

    unsubs.push(
      ctx.bus.on('event:new', (evt) => {
        send(ws, 'event:new', evt);
      }),
    );

    unsubs.push(
      ctx.bus.on('decision:new', (dec) => {
        send(ws, 'decision:new', dec);
      }),
    );

    unsubs.push(
      ctx.bus.on('decision:updated', (dec) => {
        send(ws, 'decision:updated', dec);
      }),
    );

    unsubs.push(
      ctx.bus.on('decision:approved', (dec) => {
        send(ws, 'decision:approved', dec);
      }),
    );

    unsubs.push(
      ctx.bus.on('scenario:step', (stepData) => {
